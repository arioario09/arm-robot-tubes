...
class RobotArmController:
    def __init__(self, root):
        ...
        # Gripper state tracking (local estimate)
    ...
    def deg_to_steps(self, deg, axis):
        # Hitung langkah dasar berdasarkan derajat
        steps = int((deg / self.DEG_PER_REV) * self.STEPS_PER_REV * self.gear_ratio[axis])
        
        # JIKA AXIS 2, BALIK NILAINYA MENJADI MINUS AGAR ROBOT BERGERAK MAJU
        if axis == 2:
            return -steps
    ...
    def setup_gui(self):
        ...
        # Quick buttons (Disesuaikan dengan arah derajat asli di Arduino)
        qr = ttk.Frame(gf)
        ...
        self.wp_listbox = tk.Listbox(lb_wrap, height=5, font=("Courier", 9),
        ...
        grip_opts = ttk.Combobox(gc_row, textvariable=self.wp_grip_var, width=14,
                                  values=["none","GOPEN","GCLOSE","GD 90","GD -90","GD 45","GD -45"])
        grip_opts.pack(side="left", padx=4)
        ttk.Label(gc_row, text="(atau ketik manual)", foreground="gray").pack(side="left")

        # Waypoint buttons
        wb1 = ttk.Frame(wf)
        wb1.grid(row=2, column=0, sticky="ew", pady=(0,4))
        for txt, cmd in [("+ Add", self.wp_add), ("Update", self.wp_update),
                         ("Delete", self.wp_delete), ("↑", self.wp_up),
                         ("↓", self.wp_down), ("Go To", self.wp_goto)]:
        ...
        ttk.Spinbox(wb2, from_=0.1, to=30.0, increment=0.5,
                    textvariable=self.delay_var, width=5).pack(side="left", padx=(2,8))
        ttk.Label(wb2, text="Repeat:").pack(side="left")
        self.repeat_var = tk.IntVar(value=1)
        ttk.Spinbox(wb2, from_=1, to=99, textvariable=self.repeat_var,
    ...
    def _make_slider(self, parent, label, var, row):
        ...
        sl = ttk.Scale(top, from_=0, to=540, variable=var, orient="horizontal",
                       command=lambda e: self.on_slider_change(e))
        ...
        for d in (-10,-5,-1,1,5,10):
            ttk.Button(bf, text=f"{'+' if d>0 else ''}{d}°", width=4,
                       command=lambda dv=d, v=var: self._adj(v, dv)).pack(side="left", padx=1)
    ...
    # ── Gripper commands (DISESUAIKAN DENGAN ARAH ARDUINO) ───
    def _grip_update_label(self):
        self.grip_pos_label.config(text=f"Posisi est: {self.gripper_pos_est} langkah")

    def grip_open(self):
        self.send_raw("GOPEN")
        # Arduino: GOPEN = -180 derajat (mengurangi langkah)
        self.gripper_pos_est -= int((180/360)*2048)
        self._grip_update_label()

    def grip_close(self):
        self.send_raw("GCLOSE")
        # Arduino: GCLOSE = +180 derajat (menambah langkah)
        self.gripper_pos_est += int((180/360)*2048)
        self._grip_update_label()
    ...
    def grip_deg(self, sign):
        try:
            ...
            self.send_raw(f"GD {deg:.1f}")
            self.gripper_pos_est += int((deg/360)*2048)
            self._grip_update_label()
        except ValueError:
            messagebox.showwarning("Input", "Masukkan angka derajat yang valid.")

    def grip_steps(self, sign):
        try:
            steps = int(self.grip_step_var.get()) * sign
            self.send_raw(f"GR {steps}")
            self.gripper_pos_est += steps
            self._grip_update_label()
        except ValueError:
            messagebox.showwarning("Input", "Masukkan angka langkah yang valid.")

    def grip_set_speed(self):
        rpm = self.grip_spd_var.get()
        self.send_raw(f"GSPD {rpm}")

    def grip_reset(self):
        self.send_raw("GRESET")
        self.gripper_pos_est = 0
        self._grip_update_label()

    def _send_grip_cmd(self, cmd):
        """Kirim gripper command dan update estimasi posisi sesuai fungsi Arduino."""
        if not cmd or cmd.lower() == "none":
            return
        self.send_raw(cmd)
        cmd_up = cmd.upper().strip()
        
        # Penyesuaian kalkulasi estimasi waypoint playback dengan arah Arduino
        if cmd_up == "GOPEN":
            self.gripper_pos_est -= int((180/360)*2048)
        elif cmd_up == "GCLOSE":
            self.gripper_pos_est += int((180/360)*2048)
            ...
            try:
                self.gripper_pos_est += int(cmd_up[3:])
            except: pass
    ...
    def send_homing(self):
        if messagebox.askyesno("Homing", "Jalankan homing semua axis?"):
            self.send_raw("HALL")
    ...
    def _refresh_listbox(self):
        ...
        for i, (t1,t2,t3,gc) in enumerate(self.waypoints):
            self.wp_listbox.insert("end", self._wp_label(i,t1,t2,t3,gc))

    def wp_add(self):
        t1,t2,t3 = self.theta1.get(), self.theta2.get(), self.theta3.get()
        gc = self.wp_grip_var.get().strip()
        self.waypoints.append((t1,t2,t3,gc))
        idx = len(self.waypoints)-1
        self.wp_listbox.insert("end", self._wp_label(idx,t1,t2,t3,gc))
        self.wp_listbox.selection_clear(0,"end")
    ...
    def wp_update(self):
        ...
        t1,t2,t3 = self.theta1.get(), self.theta2.get(), self.theta3.get()
        gc = self.wp_grip_var.get().strip()
        self.waypoints[i] = (t1,t2,t3,gc)
        self.wp_listbox.delete(i)
        self.wp_listbox.insert(i, self._wp_label(i,t1,t2,t3,gc))
        self.wp_listbox.selection_set(i)
    ...
    def wp_goto(self):
        ...
        self.theta1.set(t1); self.theta2.set(t2); self.theta3.set(t3)
        self.wp_grip_var.set(gc)
        self.on_slider_change(None)
        self.send_to_robot()
    ...
    def wp_play(self):
        if not self.waypoints:
            messagebox.showwarning("Waypoints", "Belum ada waypoint."); return
    ...
    def _play_loop(self):
        ...
        for _ in range(repeat):
            ...
            for i, (t1,t2,t3,gc) in enumerate(self.waypoints):
                ...
                # Kirim axis
                s1 = self.deg_to_steps(t1,1)
                s2 = self.deg_to_steps(t2,2)
                s3 = self.deg_to_steps(t3,3)
                self.root.after(0, self.send_raw, f"G {s1} {s2} {s3}")
                # Tunggu sebentar lalu kirim gripper (beri waktu axis bergerak)
                time.sleep(delay * 0.6)
                if gc and gc.lower() != "none":
                    self._send_grip_cmd(gc)
                    # Gripper 28BYJ-48 butuh waktu: estimasi 1s per 90° di 10RPM
                    try:
                        deg_val = 90.0
                        if "GD" in gc.upper():
                            deg_val = abs(float(gc.split()[-1]))
                        elif gc.upper() in ("GOPEN","GCLOSE"):
                            deg_val = 180.0
                        extra = max(0.5, deg_val / 90.0 * 1.0)
                    except:
                        extra = 1.0
                    time.sleep(extra)
                else:
    ...
    def on_slider_change(self, event):
        for var in (self.theta1, self.theta2, self.theta3):
            if hasattr(var, '_lbl'):
                var._lbl.config(text=f"{var.get():.1f}°")
        s1 = self.deg_to_steps(self.theta1.get(), 1)
        s2 = self.deg_to_steps(self.theta2.get(), 2)
        s3 = self.deg_to_steps(self.theta3.get(), 3)
        self.steps_label.config(text=f"G  {s1}   {s2}   {s3}")
        self.update_plot()

    # ── FK ────────────────────────────────────────────────────
    def fk(self, t1d, t2d, t3d):
        t1,t2,t3 = np.radians(t1d), np.radians(t2d), np.radians(t3d)
        p0 = np.array([0,0,0])
    ...
    def update_plot(self):
        ...
        ax.plot(xs,ys,zs,'o-', color='royalblue', linewidth=3,
                markersize=8, markerfacecolor='white', markeredgecolor='royalblue')
        ...
        ax.set_zlim([0, self.L1+self.L2+self.L3])
        ax.set_xlabel('X',fontsize=8); ax.set_ylabel('Y',fontsize=8); ax.set_zlabel('Z',fontsize=8)
        ax.set_title(f"θ1={self.theta1.get():.0f}°  θ2={self.theta2.get():.0f}°  θ3={self.theta3.get():.0f}°"
                     f"   EE=({xs[-1]:.2f}, {ys[-1]:.2f}, {zs[-1]:.2f})", fontsize=8)
        ...
        for ax2, hh, vv, title, xl, yl in [
            ...
            ax2.clear()
            ax2.plot(hh, vv, 'o-', color='royalblue', linewidth=2,
                     markersize=5, markerfacecolor='white', markeredgecolor='royalblue')
            ax2.scatter([hh[-1]],[vv[-1]], c='green', s=40, zorder=5)
            ax2.set_title(title, fontsize=7)
